...
const About = () => {
  ...
  useEffect(() => {
    const fetchExperiences = async () => {
      try {
        ...
        console.warn("Could not fetch experiences:", err.message);
      } finally {
        setLoadingExp(false);
      }
    };
  ...
  return (
    <>
      {modalOpen && experiences.length > 0 && (
        <ExperienceModal
          ...
          <div ref={fadeRef} className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14 items-start fade-in">
            ...
            <div>
              <div style={{ fontSize: 13, color: "#6a9bbf", lineHeight: 1.9 }}>
                ...
                <p>
                  ...
                  architecture, ensuring every solution is both functional and maintainable.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 gap-2.5 my-5">
                {[
                  ...
                  <div key={item.label} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg"
            ...
            <div className="grid grid-cols-2 gap-3">
              {stats.map((s) => (
                <div key={s.label}
                  onClick={() => {
                    if (s.clickable && experiences.length > 0) {
                  ...
                  style={{
                    background: "#0c1422",
                    border: `1px solid ${s.clickable ? "rgba(0,229,255,0.25)" : "rgba(0,229,255,0.1)"}`,
                    padding: "16px",
                    cursor: s.clickable ? "pointer" : "default",
                  }}
                  onMouseEnter={(e) => { if (s.clickable) { e.currentTarget.style.borderColor = "#00e5ff"; e.currentTarget.style.boxShadow = "0 0 20px rgba(0,229,255,0.1)"; e.currentTarget.style.transform = "translateY(-2px)"; } }}
                  onMouseLeave={(e) => { if (s.clickable) { e.currentTarget.style.borderColor = "rgba(0,229,255,0.25)"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = "translateY(0)"; } }}>
                  {s.clickable && (
